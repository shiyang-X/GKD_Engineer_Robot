...
namespace Config
{
    ...
    const std::vector<std::string> CanInitList = { "CAN_GIMBAL", "CAN_CHASSIS"};
    ...
    const std::vector<std::tuple<std::string, int, int>> SerialInitList = {
        ...
        {"/dev/ARM_RX",921600,2000},
        {"/dev/ARM_TX",115200,2000}
    };

    const std::string rc_controller_serial = "/dev/IMU_HERO";
    const std::string super_cap_can_interface = "CAN_CHASSIS";

    const std::string arm_serial_rx = "/dev/ARM_RX";
    const std::string arm_serial_tx = "/dev/ARM_TX";
